...
const PidNavigation = ({currentProject}:any) => {
  return (
    <div className="w-full md:mt-12 px-4 md:px-24 z-10 snap-start gap-6">
      <div className="flex py-6 justify-center md:justify-between items-center gap-4 flex-wrap">
        ...
        <ul className="flex flex-col md:flex-row gap-6 text-stone-500 text-center">
          {projects.map((project, index) => {
            return (
              <li key={index} className={`hover:underline ${currentProject === project ? "underline" : ""}`}><Link href={`/${project.id}`}>{project.project_name}</Link></li>
            )
          })}
        </ul>
      </div>
